...
Remaining planned coverage — backend auth logic, API route integration tests via Supertest (which first needs the app split out of `server.ts` and the native `bcrypt` import chain resolved), and frontend component tests (Vitest + React Testing Library, scoped to four tests) — is tracked in `nextSteps.md`.
