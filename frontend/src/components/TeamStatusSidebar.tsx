...
const TeamStatusSidebar = () => {
  ...
  const { members, toggleAvailability } = useTeam();
  const [currentUser, setCurrentUser] = useState<string>('');

  // Keeps track of which active user layout to simulate
  useEffect(() => {
    if (members.length > 0 && !currentUser) {
      setCurrentUser(members[0]._id); // Fallback to first member for active user simulation
    }
  }, [members, currentUser]);
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {members.map((member: any) => {
          const isSelf = member._id === currentUser;
          return (
